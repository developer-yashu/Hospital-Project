  ...
  const apiget = async () => {
    try {
      const res = await axios.get(
        `http://127.0.0.1:1010/location/get-state?search=${search}`,
      );
    ...
    <div className="p-6 min-h-screen bg-gray-50">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {data.length > 0 ? (
          data.map((item) => (
            ...
            >
              ...
              <div className="flex items-start justify-between mb-3">
                <div
                  className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg ${item.status === "inactive" ? "bg-red-50" : "bg-blue-50"}`}
                >
                  🗺️
                </div>
          ...
          <p className="text-gray-400 text-sm col-span-4 text-center py-12">
            No state found.
          </p>
        )}
      </div>
          ...
          <div className="bg-white p-6 rounded-2xl w-80 border border-gray-100">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center py-2 text-sm">
                ...
                <span
                  className={`text-xs font-medium px-2.5 py-1 rounded-full ${singleState.status === "inactive" ? "bg-red-50 text-red-700" : "bg-green-50 text-green-700"}`}
                >
                  {singleState.status === "inactive"
                    ? "● Inactive"
                    : "● Active"}
                </span>
              </div>
