...
const { hospitalId } = useParams();
  ...
  const GetHospital = async () => {
    try {
      ...
      const res = await axios.get(`http://127.0.0.1:1010/Hospital/get-one-hospital/${hospitalId}`);
      console.log("get-one-hospital", res.data.hospitals);
      setHospital(res.data.hospitals);
      setLoading(false);
    } catch (error) {
      console.log(error);
      alert(error.response?.data?.message || "Failed to fetch hospital details");
    }  
  };
        ...
        <div className="bg-gradient-to-r from-blue-600 to-cyan-500 p-8 flex flex-col lg:flex-row gap-8 items-center">
          ...
          <div className="flex-1 text-white">
            ...
            <div className="space-y-3 text-lg">
              ...
              <p className="flex items-center gap-2">
                <span className="text-2xl">📍</span>
                {hospital.CityId?.city},{" "}
                {hospital.districtId?.district},{" "}
            ...
            doctors.map((item) => (
              ...
              >
                ...
                <div className="pt-20 pb-6 px-6">
                  ...
                  {/* BUTTON */}
                  <button className="mt-6 w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-2xl font-semibold transition duration-300 shadow-md">
                    approved
                  </button>
                </div>
              </div>
